...
async def illuminate_cell(pixels, interval):  # Don't forget the async!
    leds = [0,1,2,3,9,8,7,6,10,11,12,13]
    led = 11
    while True:
        next_led = (led + 1) % 12
        pixels[leds[led]] = (0,0,0)
        pixels[leds[next_led]] = (0,150,0)
        led = next_led
        await asyncio.sleep(interval)  # Don't forget the await!
...
async def main():  # Don't forget the async!
    ...
    # led_task = asyncio.create_task(illuminate_cell(pixels, 2))
    # status_update_task = asyncio.create_task(update_status(5))
    check_button_task = asyncio.create_task(check_buttons(rtc, board.D1))
    # await asyncio.gather(status_update_task, check_button_task, led_task)  # Don't forget the await!
    await asyncio.gather(check_button_task)  # Don't forget the await!
    print("done")
